...
class PopupGrid {
  ...
  void reqApisPopup(type) {
    ...
    if (rows.isEmpty) {
      List<dynamic>? apis = [];
      if (type == 'StackGridItem') {
        apis = selectedApis?.values
            .where(
                (e) => e['if_id'] != properties['content']['apiId'].toString())
            .toList();
      } else {
        apis = selectedApis?.values.toList();
      }
      apis?.forEach((e) {
  ...
  void reqMenusPopup(type) {
    ...
    if (rows.isEmpty) {
      final defaultMenus = [
        {'menuId': 'home', 'label': '홈', 'icon': 'Icons.home'},
        {'menuId': 'settings', 'label': '설정', 'icon': 'Icons.settings'},
        {'menuId': 'profile', 'label': '프로필', 'icon': 'Icons.person'},
        {'menuId': 'dashboard', 'label': '대시보드', 'icon': 'Icons.dashboard'},
        {'menuId': 'list', 'label': '목록', 'icon': 'Icons.list'},
      ];
            ...
            child: FormBuilder(
              ...
              child: FormBuilderDropdown<String>(
                ...
                onChanged: (v) {
                  ...
                  old.cells['icon']?.value = v;
                  if (old.cells['label']?.value.isEmpty) {
                    old.cells['label']?.value =
                        icons.firstWhere((icon) => icon['value'] == v)['label'];
                    old.cells['menuId']?.value =
                        '${icons.firstWhere((icon) => icon['value'] == v)['value'].split('.').last}';
                  }
                },
              ),
            ),
