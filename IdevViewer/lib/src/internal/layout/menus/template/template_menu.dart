...
class _TemplateMenuState extends State<TemplateMenu> {
  ...
  void _handleApiIdResponse(dynamic v) async {
    ...
    final type = result?.runtimeType.toString() ?? 'null';

    if (v['if_id'] == '${ApiEndpointIDE.templateCategories}/parent') {
      templates = result == null
          ? []
          : type.contains('JsonMap')
              ? [result]
              : (result as List);
      ...
      categories = result == null ? [] : (result as List);
  ...
  Future<List<TrinaRow>> loadTemplateList() async {
    ...
    //debugPrint('templates: $templates');
  ...
  TrinaRow templateItem(Map e) {
    return TrinaRow(cells: {
      ...
      'description': TrinaCell(value: e['template']?['description'] ?? ''),
                      ...
                      child: Stack(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              Text(BuildMode.isViewer ? '뷰어' : '템플릿'),
                            ],
                          ),
                          Positioned(
                              right: 0,
      ...
      child: TrinaGrid(
        ...
        onSelected: (event) {
          ...
          Future.delayed(const Duration(milliseconds: 500), () {
            popupTemplateBas(data: event.row?.toJson()).then((value) {
              if (value != null) {
  ...
  Future<dynamic> popupTemplateBas({Map<String, dynamic>? data}) async {
    //debugPrint('popupTemplateBas json: $data');
                      ...
                      children: [
                        ...
                        if (data != null && commitRows.isNotEmpty)
                          InkWell(
                              ...
                              child: const Row(
                                spacing: 4,
                                children: [Icon(Icons.delete), Text('커밋 삭제')],
                              )),
                        InkWell(
                            onTap: () {
    ...
    if (result == null) {
      ...
      List<Map<String, dynamic>> toJson =
          homeRepo.hierarchicalControllers[homeRepo.currentTab]?.getAllData() ??
              [];
      if (toJson.isEmpty) {
    ...
    String deleteTarget = (isDelete && commitRows.isNotEmpty) ? 'commit' : '';
  ...
  void execTemplate(
    ...
    homeRepo.addJsonMenuState(jsonData);
  }
}
